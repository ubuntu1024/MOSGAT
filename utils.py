...
def Normal_value_of_sample(sample_data,label_list,num_view):
    Num_samp = sample_data[0].shape[0]
    Normal_samp_list = []
    Nol_samp_idex = []
    Nol_sam_value_list = []
    for i in range(Num_samp):
        if label_list[i] == 0:
          Nol_samp_idex.append(i)
    for i in range(num_view):
        Normal_samp_list.append(sample_data[i][Nol_samp_idex].clone())
    for i in range(num_view):
            Nol_sam_value_list.append(Normal_samp_list[i].mean(axis=0))
    return Nol_sam_value_list

def Min_MAX_Scaler(data):
    min_data = torch.min(data)
    max_data = torch.max(data)
    returndata = (data-min_data)/(max_data-min_data)
    return returndata

def Samp_pro_tensor(data_list,Normal_samp_value_list,Num_sam,num_view):
    processed_list = []
    for i in range(num_view):
        tem_list = []
        for j in range(Num_sam):
            tem_list.append(abs(data_list[i][j]-Normal_samp_value_list[i]))
        tem_list = torch.stack(tem_list)
        processed_list.append(tem_list)
